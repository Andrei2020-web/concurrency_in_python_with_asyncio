...
async def retry(coro: Callable[[], Awaitable],
    ...
    for retry_num in range(0, max_retries):
        try:
            ...
            # Если получено исключение, протоколировать его и ждать в течение заданного интервала
            # перед повторной попыткой
            logging.exception(
                f'Во время ожидания произошло исключение (попытка {retry_num}), пробую ещё раз.',
                exc_info=e)
