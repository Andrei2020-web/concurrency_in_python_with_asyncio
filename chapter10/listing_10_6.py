...
from chapter10.listing_10_7 import retry
...
async def all_products(request: Request) -> Response:
    async with aiohttp.ClientSession() as session:
        ...
        cart_request = functools.partial(
            session.get, f'{CART_BASE}/users/3/cart')
        ...
        # Если запрос к сервису товаров завершился по тайм-ауту,
        # вернуть ошибку, потому что продолжение невозможно
        if products in pending:
            ...
            [request.cancel() for request in requests]
            logging.exception('Ошибка сервера при подключении к серверу товаров.',
                              exc_info=products.exception())
            return web.json_response(
                {'error': 'Ошибка сервера при подключении к сервису товаров'}, status=500)
        else:
            ...
            product_results: List[Dict] = await get_products_with_inventory(session,
                                                                            product_response)
            cart_item_count: Optional[int] = await get_response_item_count(cart, done,
                                                                           pending,
                                                                           'Ошибка ошибка получения корзины пользователя')
            favorite_item_count: Optional[int] = await get_response_item_count(favorites,
                                                                               done,
                                                                               pending,
                                                                               'Ошибка получения избранных товаров пользователя')
            return web.json_response({'cart_items': cart_item_count,
                                      'favorite_items': favorite_item_count,
                                      'products': product_results})


async def get_products_with_inventory(session: ClientSession, product_response) -> List[
    Dict]:
    ...
    for done_task in inventory_done:
        if done_task.exception() is None:
            ...
            logging.exception(f'Ошибка получения количества для товара {product_id}',
                              exc_info=inventory_tasks_to_product_id[
                                  done_task].exception())
