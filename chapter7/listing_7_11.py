...
from chapter7.listing_7_10 import StressTest
...
class LoadTester(Tk):
    ...
    def _update_bar(self, pct: int):
        '''Устанавливает процент заполненности индикатора хода выполнения от 0 до 100.
    ...
    def _queue_update(self, completed_requests: int, total_requests: int):
        '''Метод является обратным вызовом, который передается нагрузочному тесту;
