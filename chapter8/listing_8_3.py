...
class ServerState:
    ...
    async def _echo(self, reader: StreamReader, writer: StreamWriter):
        ...
        try:
            ...
            await self._notify_all(f'Клиент отключился. Осталось пользователей:'
                                   f'{len(self._writers)}!\n')
        except Exception as e:
            logging.exception('Ошибкачтения данных от клиента.', exc_info=e)
            self._writers.remove(writer)


async def _notify_all(self, message: str):
    # Вспомогательный метод для отправки сообщения всем остальным пользователям.
